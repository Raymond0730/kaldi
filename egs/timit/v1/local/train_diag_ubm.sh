...
if $apply_cmn; then
  ...
  #test="ark,s,cs:add-deltas $delta_opts scp:$test_dir/feats.scp ark:- | apply-cmvn-sliding --norm-vars=false --center=true --cmn-window=300 ark:- ark:- | select-voiced-frames ark:- scp,s,cs:$test_dir/vad.scp ark:- | subsample-feats --n=$subsample ark:- ark:- |"
else
...
for x in `seq 0 $[$num_iters-1]`; do
  ...
  # Accumulate stats.
    ...
    $cmd $dir/log/update.$x.log \
      gmm-global-est $opt --min-gaussian-weight=$min_gaussian_weight $dir/$x.dubm "gmm-global-sum-accs - $dir/$x.*.acc|" \
      $dir/$[$x+1].dubm || exit 1;
    #gmm-global-get-frame-likes --average=true $dir/$x.dubm "$test" ark,t:$results/$x.like

    $cleanup && rm $dir/$x.*.acc $dir/$x.dubm
  fi
done
#gmm-global-get-frame-likes --average=true $dir/$num_iters.dubm "$test" ark,t:$results/$num_iters.like
mv $dir/$num_iters.dubm $dir/final.dubm || exit 1;
cp $dir/final.dubm exp/spk_gmm/ubm
